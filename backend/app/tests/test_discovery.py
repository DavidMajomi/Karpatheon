...
class TestDiscoveryServiceIntegration:
    ...
    def test_url_deduplication(self, mock_neo4j, mock_embeddings, mock_exa):
        ...
        # Mock crawled URLs
        service.load_crawled_urls = MagicMock(return_value={
            "urls": {
                "https://already-crawled.com": {
                    "first_crawled": "2025-11-23T09:00:00Z",
                    "crawl_count": 1
                }
            }
        })
        ...
        filtered = service.filter_crawled_urls(urls)
        
        assert len(filtered) == 2
        assert "https://already-crawled.com" not in filtered
        assert "https://new-url.com" in filtered
    ...
    async def test_kb_similarity_fallback(self, mock_neo4j, mock_embeddings, mock_exa):
        ...
        mock_driver = mock_neo4j.return_value
        mock_driver.execute_query.side_effect = Exception("Neo4j connection error")
        
        # Should return default similarity instead of crashing
        embedding = [0.1] * 768
        similarity = await service.compute_kb_similarity(embedding)
        
        assert similarity == 0.5  # Default fallback value
